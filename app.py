...
# ================= 網頁設定 =================
st.set_page_config(page_title="台股波段多空篩選器 V3.1", page_icon="📈", layout="wide")
...
def calculate_kline_score(df_stock):
    ...
    O1, C1 = df_stock['Open'].iloc[-1], df_stock['Close'].iloc[-1]
    # ★ 統一轉換為張數
    V1 = df_stock['Volume'].iloc[-1] / 1000 
    V2 = df_stock['Volume'].iloc[-2] / 1000
    vol_5ma = (df_stock['Volume'].tail(5).mean()) / 1000
        ...
        if all_filtered and HAS_AI:
            ...
            if col1.button(f"✨ 診斷 {selected_stock_name}", type="primary", use_container_width=True):
                with st.spinner("AI 深度思考中..."):
                    ...
                    # ★ 傳給 AI 之前，把成交量轉成張
                    vol_today = int(df_tech['Volume'].iloc[-1] / 1000) if not df_tech.empty else 0
            ...
            if do_defense:
                with st.spinner("AI 正在解析「進場至今」的量價足跡..."):
                    ...
                    try:
                        ...
                        # ★ 成交量統一轉成張數
                        up_vol = (df_post[df_post['Close'] > df_post['Open']]['Volume'] / 1000).mean()
                        down_vol = (df_post[df_post['Close'] < df_post['Open']]['Volume'] / 1000).mean()
                    ...
                    prompt = f"""
                    你是一位極度重視風險控管的基金經理人，請根據進場收盤價以後的連續k棒進行分析，並結合籌碼給予新手詳細的引導教學。
                    ...
                    ### 1. 📊 持股現況與量價推演
                    ### 2. 🛡️ 結合各項技術指標，例如macd、kd、均線、乖離率進行分析與風險提示
                    ### 3. 🎯 防禦策略 (強制設定移動停利或停損)
                    ### 4. 💡 操盤手心法
                    """
            ...
            with st.spinner("AI 正在比對選出當天與後續的多K棒軌跡..."):
                ...
                try:
                    ...
                    for idx, r in df_post.iterrows():
                        # ★ 統一轉成張數
                        vol_lots = int(r['Volume'] / 1000)
                        kline_history_str += f"- {idx.strftime('%m/%d')}: 開{r['Open']:.2f}, 高{r['High']:.2f}, 低{r['Low']:.2f}, 收{r['Close']:.2f}, 量{vol_lots}張\n"
                        
                except: kline_history_str = "無法取得連續 K 線資料"
                
                prompt = f"""
                你是一位嚴厲但專業的量化策略檢討導師。我們現在要進行事後覆盤，請結合連續的k棒與籌碼進行盤勢分析。
                ...
                請針對這筆交易進行法醫解剖，並且嚴格教導：
                ### 1. 📊 多K棒連續型態與籌碼印證
                (請分析進場後的這幾天，走出了什麼經典 K 棒組合型態？並對比「當初突破日的籌碼快照」，如果當初法人有買，為何後來走弱？)
                ### 2. 🕵️‍♂️ 成功或失敗的核心原因
                ### 3. 🛡️ 應對的出場時機檢討
                ### 4. 💡 策略進化總結
                """
